function [DupdateK,sgsimporosity,faultout] = ESMDA_Localization2 (sgactual,sgporoactual,faulten,f, N, Sim1,alpha,tol,indices,c);
%-----------------------------------------------------------------------------
...
Cyd=(yprime*S')./((N-1));
Cdd=(S*S')./((N-1));
%Cyd=yoboschur.*Cyd; % Covariance localization done here')
...
tol=toluse(1,:);
disp('  update the new ensemble  ');
Ynew=Y+yoboschur.*((Cyd)*pinv((Cdd+(alpha.*Cd2)))*(Djuse-Simuse));
